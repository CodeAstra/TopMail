...
private
  def get_message_from_dump(message_dump)
    ...
    message[:to].each do |recepient|
      message[:to][recepient[:email]][:thumbnail] = message_dump.person_info[recepient[:email]]["thumbnail"]
    end
  ...
  def message_was_from_user(message, user)
    !message[:to].collect{|receipient| receipient[:email]}.include?(user[:email])
  end
end
